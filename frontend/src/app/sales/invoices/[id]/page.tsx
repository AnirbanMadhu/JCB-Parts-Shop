        ...
        <div className="max-w-7xl mx-auto p-6">
          ...
          >
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={handlePrint}
              className="px-6 py-3 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 font-semibold shadow-lg hover:shadow-xl transition-all duration-200 flex items-center gap-2"
            >
              ...
              >
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-sm font-semibold text-foreground mt-1">{invoice.customer?.name || 'N/A'}</p>
                  </div>
                  {invoice.customer?.indexId && (
                    <div>
                      <span className="text-xs font-medium text-muted-foreground">Customer ID</span>
                      <p className="text-sm font-mono text-foreground mt-1">{invoice.customer.indexId}</p>
                    </div>
                  )}
                  {invoice.customer?.phone && (
                    <div>
                      <span className="text-xs font-medium text-muted-foreground">Phone</span>
                      <p className="text-sm text-foreground mt-1">{invoice.customer.phone}</p>
                    </div>
                  )}
                  {invoice.customer?.gstin && (
                    <div>
                      <span className="text-xs font-medium text-muted-foreground">GSTIN</span>
                      <p className="text-sm text-foreground mt-1">{invoice.customer.gstin}</p>
                    </div>
                  )}
                </div>
              </motion.div>
            ...
            >
              <div className="flex justify-end">
                <div className="w-full md:w-96 space-y-3">
                  ...
                  {(invoice.discountPercent > 0 || (invoice.discountAmount && Number(invoice.discountAmount) > 0)) && (
                    <div className="flex justify-between text-sm border-t border-border pt-2">
                      <span className="text-muted-foreground">
                        Discount {invoice.discountPercent > 0 ? `(${invoice.discountPercent}%)` : ''}:
                      </span>
                      <span className="font-semibold text-green-600 dark:text-green-400">- ₹{Number(invoice.discountAmount).toFixed(2)}</span>
                    </div>
                  )}
                  <div className="flex justify-between text-sm pb-3 border-t border-border pt-2">
