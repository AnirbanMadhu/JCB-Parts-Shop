      ...
      <header className="sticky top-0 z-10 bg-card border-b border-border px-3 sm:px-6 py-3 sm:py-3.5 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <h1 className="text-sm sm:text-[17px] font-semibold text-foreground truncate">Sales Invoice</h1>
          </div>
        </div>
        <Link href="/sales/invoices/new" className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 text-xs sm:text-sm bg-primary text-primary-foreground rounded-md hover:bg-primary/90 transition-all shadow-sm hover:shadow-md touch-manipulation flex-shrink-0">
        ...
        <div className="bg-card border border-border rounded-lg overflow-hidden shadow-sm animate-fade-in">{!hasRows ? (
            ...
            <>
              ...
              <div className="lg:hidden">
                {invoices.map((inv, i) => (
                  <div key={inv.id} className="p-4 border-b border-border hover:bg-muted/20 transition-colors">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1 min-w-0">
                        <Link href={`/sales/invoices/${inv.id}`} className="text-primary hover:underline font-medium text-sm sm:text-base block mb-1 truncate">
                          {inv.invoiceNumber}
                        </Link>
                        <p className="text-xs sm:text-sm text-muted-foreground truncate">{inv.customer?.name || '-'}</p>
                      </div>
                      <span
                    ...
                    {isLoaded && (settings.sales.allowEditSubmitted || inv.status === "DRAFT") && (
                      <div className="mt-3 pt-3 border-t border-border">
                        <Link
                          href={`/sales/invoices/${inv.id}/edit`}
                          className="text-xs sm:text-sm text-primary hover:underline transition-colors touch-manipulation inline-block"
                        >
                          Edit Invoice
                        </Link>
                      </div>
