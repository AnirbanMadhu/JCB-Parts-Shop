...
export default function PurchasePaymentsList({ payments }: Props) {
  ...
  const filteredPayments = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      filtered = filtered.filter(payment => 
        ...
        payment.supplier?.name.toLowerCase().includes(term) ||
  ...
  return (
    <div className="flex flex-col min-h-[100dvh] bg-background">
      {/* Header - Fixed */}
      <header className="sticky top-0 z-30 bg-card/95 backdrop-blur border-b border-border px-3 sm:px-6 py-3 sm:py-3.5 flex items-center justify-between gap-3 shadow-sm">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
          <BackButton />
          <div className="flex items-center gap-2 min-w-0">
            <span className="w-1 h-5 sm:h-6 bg-primary rounded-full flex-shrink-0" />
            <h1 className="text-sm sm:text-[17px] font-semibold text-foreground truncate">Purchase Payments</h1>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          <Link href="/purchases/invoices/new" className="px-3 sm:px-4 py-1.5 text-xs sm:text-sm bg-primary text-primary-foreground rounded-md hover:bg-primary/90 transition-all shadow-sm hover:shadow-md whitespace-nowrap touch-manipulation">
          ...
          <div className="relative md:col-span-2">
            ...
            {searchTerm && (
              <button
                ...
                className="absolute right-3 top-1/2 transform -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center -mr-2 touch-manipulation"
              >
                ✕
              </button>
            )}
        ...
        <div className="flex gap-2 mt-3 sm:mt-4 overflow-x-auto pb-2 hide-scrollbar">
          {['ALL', 'PAID', 'SUBMITTED', 'DRAFT', 'CANCELLED'].map(status => (
            <button
              ...
              className={`px-3 sm:px-4 py-2 text-xs sm:text-sm rounded-md transition-all whitespace-nowrap flex-shrink-0 touch-manipulation ${
                statusFilter === status
      ...
      <div className="px-3 sm:px-6 py-3 sm:py-6 flex-1 flex flex-col min-h-0">
        <div className="bg-card border border-border rounded-lg overflow-hidden shadow-sm flex flex-col flex-1">
          {!hasRows ? (
            ...
            <div className="flex flex-col items-center justify-center flex-1 py-16 px-4">
              ...
              <Link href="/purchases/invoices/new" className="px-5 py-2 bg-primary text-primary-foreground text-sm font-medium rounded-md hover:bg-primary/90 transition-all shadow-sm hover:shadow-md touch-manipulation">
                  ...
                  <div key={payment.id} className="p-4 border-b border-border hover:bg-muted/50 active:bg-muted/70 transition-colors mobile-card">
                    <div className="flex items-start justify-between mb-3 gap-2">
                      <div className="min-w-0 flex-1">
                        <Link href={`/purchases/invoices/${payment.id}`} className="text-blue-600 hover:underline font-medium text-sm block mb-1 truncate touch-manipulation">
                          {payment.invoiceNumber}
                        </Link>
                    ...
                    {payment.note && (
                      <div className="mt-2 pt-2 border-t border-border">
                        ...
                        <span className="text-xs text-foreground break-words">{payment.note}</span>
                      </div>
                    )}
                  </div>
                ))}
